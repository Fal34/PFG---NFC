...
Un cuerpo finito de \textit{Galois} $GF(n)$ es un cuerpo finito generado por $n$, siendo $n$ primo \cite{lucena}. Por lo que se forman pares de polinomios de grado $n-1$ con coeficientes binarios, fácilmente representables por cadenas de bits\cite{lucena}. %Se define $E(GF(2^n))$ como los puntos dentro de un cuerpo finito dados por una curva elíptica generados por un número en base 2 exponenciado por otro número primo alto.
